...
import 'package:new_proj/GamesPage.dart';
...
class _LoginPageState extends State<LoginPage> { //state management class for the 'LoginPage' widget+
  ...
  final passwordController = TextEditingController(text: "rootpass"); //controller that manages the input for the password
  final ipController = TextEditingController(text: "10.0.2.2"); //controller that manages input for the IP address
  ...
  validateLogin() async {
    ...
    if(response.statusCode == 200){ //if the response is successful
      ...
      if(jsonData["success"]){ //if login is successful then stores credentials in SharedPreferences
        ...
        await prefs.setInt('IDUtilizador', jsonData["IDUtilizador"]);
        ...
        showDialog(
          ...
          builder: (context) {
            return const AlertDialog(
              content: Text("Username or Password incorrects"),
            );
          },
        );
      }
