...
MQTT_PORT = 1883
TOPICO_MOV = "mov"  # Tópico para enviar dados de movimento
...
def enviar_para_mqtt(dados):
    ...
    for dado in dados:
        ...
        comando = f'mosquitto_pub -h {MQTT_BROKER} -t {TOPICO_MOV} -q 2 -m "{mensagem_json}"'
        subprocess.run(comando, shell=True)

        print(f"✅ Mensagem enviada para '{TOPICO_MOV}' (QoS 2): {mensagem_json}")


# Loop principal
if __name__ == "__main__":
    print(f"mongo_mov iniciou")
    player_id = 50  # Substitua com o ID do jogador
    while True:
        dados_mov = buscar_e_remover_dados_mongo(f"pisid_mazemov_{player_id}")
        enviar_para_mqtt(dados_mov)
        time.sleep(5)
